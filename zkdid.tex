...
\begin{itemize}
    \item \textbf{Domain-specific ZKP circuit design}: The first formalization of Islamic zakat eligibility predicates (\textit{nisab}, \textit{hawl}, \textit{asnaf} classification) as R1CS arithmetic circuits for zk-SNARK proving.
...
\begin{enumerate}
    ...
    \item \textbf{Domain-specific ZKP circuits}: Existing ZKP credential frameworks \cite{pauwels2021, camenisch2012} are designed for generalized financial compliance and do not encode Islamic jurisprudential eligibility criteria---\textit{nisab} thresholds, \textit{hawl} assessment periods, and the eight \textit{asnaf} categories.
...
\midrule
2. Proof Generation & Mustahik Claimant & Selects applicable \textit{asnaf} category; invokes ZKP circuit $C$. Uses VC attributes as private witness to generate zk-SNARK proof $\pi$ and nullifier hash $\eta = H(\text{DID} \| \text{cycle})$. \\
\midrule
